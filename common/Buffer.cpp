...
Buffer::Buffer(const int size): m_capacity(size) {
...
void Buffer::extendRoom(const int size) {
    // 1. 内存够用 - 不需要扩容
    ...
    // 3. 内存不够用 - 扩容
        ...
        memset(static_cast<char *>(temp) + m_capacity, 0, size);
        // 更新数据
...
char *Buffer::findCRLF() {
    auto ptr = static_cast<char *>(memmem(m_data + m_readPos, readableSize(), "\r\n", 2));
    return ptr;
}
